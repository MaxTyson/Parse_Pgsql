...
try:
    ...
    print('Tried to connect with database.')
cur = conn.cursor()
...
def add_category_table():
    ...
    for category in DATA['categories']:
        cur.execute(insert_category(category, DATA['ids_url']
                                    .format(category)))
    return conn.commit()
...
def insert_item(category_name, *args):
    # If data doesn't exist in DB - do INSERT, otherwise do UPDATE
    cur.execute(""" INSERT INTO item (item_id, author, score, time, title,
                type, url, category_fk_id)
                VALUES('{0}', '{1}', '{2}', '{3}', '{4}', '{5}', '{6}',
                '{7}') ON CONFLICT (item_id) DO UPDATE SET (author, score,
                time, title, url) = (EXCLUDED.author, EXCLUDED.score,
                EXCLUDED.time, EXCLUDED.title, EXCLUDED.url);
                """.format(args[0],
                           args[1],
                           args[2],
                           args[3],
